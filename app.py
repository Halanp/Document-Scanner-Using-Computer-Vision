...
def detect_document(image, edges, mask=None):
    ...
    for contour in contours:
        ...
        # Rectangle area
        # ======================================================
        rect_area = cv2.contourArea(points)
        ...
        # Reject extremely strange shapes
        if aspect_ratio > 5.0:
            continue
        ...
        # Smaller distance = closer to image boundary
        border_score = 1.0 - min(
            max(border_distance, 0) /
            (0.30 * min(h, w)),
            1.0
    ...
    if candidates:
        ...
        best_score, best_ratio, best_points = candidates[0]

        document = order_points(
            best_points
        )
    ...
    if mask is not None:
        ...
        document = document_from_mask(
            mask
        )

        if document is not None:

            mask_area = cv2.contourArea(
                document.astype(np.float32)
            )

            if mask_area > image_area * 0.40:
                return order_points(
                    document
                )
